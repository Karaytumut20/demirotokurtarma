...
import { CheckCircle2, MapPin, Phone, ShieldCheck, Clock, ArrowLeft } from 'lucide-react';
            ...
            <div className="lg:w-2/3">
                <div className="bg-white p-6 sm:p-10 rounded-3xl shadow-sm border border-slate-100">
                    ...
                    <div className="prose prose-lg text-gray-600 leading-relaxed mb-10">
                        <p className="font-semibold text-slate-800 text-lg mb-4">{pageData.content.intro}</p>
                        <p>{pageData.content.details}</p>
                    </div>

                    <h3 className="text-xl font-bold text-[#0f172a] mb-4">Hizmet Özellikleri</h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        {pageData.content.features.map((feature, idx) => (
                            <div key={idx} className="flex items-center gap-3 p-4 bg-blue-50/50 rounded-xl border border-blue-100 hover:border-blue-300 transition-colors">
                                <CheckCircle2 className="text-blue-600 shrink-0" size={20} />
                                <span className="font-bold text-slate-700 text-sm sm:text-base">{feature}</span>
                            </div>
                        ))}
                    </div>
                </div>
                ...
                <div className="mt-8 grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6">
                    <div className="bg-white p-6 rounded-2xl shadow-sm text-center border border-slate-100">
                        ...
                        <p className="text-xs text-gray-500 mt-1">Bölgeye hakim ekipler.</p>
                    </div>
                    <div className="bg-white p-6 rounded-2xl shadow-sm text-center border border-slate-100">
                        <ShieldCheck className="w-10 h-10 text-green-600 mx-auto mb-3" />
                        <h3 className="font-bold text-slate-900">Kaskolu Taşıma</h3>
                        <p className="text-xs text-gray-500 mt-1">Axa Sigorta güvencesi.</p>
                    </div>
                    <div className="bg-white p-6 rounded-2xl shadow-sm text-center border border-slate-100">
            ...
            <div className="lg:w-1/3">
                <div className="sticky top-32 space-y-6">
                    ...
                    <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                        <h4 className="font-bold text-lg mb-4 pb-2 border-b border-slate-100 text-[#0f172a]">Diğer Hizmet Bölgeleri</h4>
                        <ul className="space-y-2">
                            {locationPages.filter(p => p.slug !== resolvedParams.slug).map(p => (
                                <li key={p.slug}>
                                    <Link href={`/bolge/${p.slug}`} className="flex items-center gap-3 p-3 rounded-lg hover:bg-slate-50 text-gray-600 hover:text-blue-600 transition-all group">
