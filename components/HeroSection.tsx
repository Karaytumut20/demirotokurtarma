    ...
    <div className="relative min-h-[100dvh] w-full flex items-center justify-center overflow-hidden bg-[#020617]">
      ...
      <div className="relative z-20 container mx-auto px-4 sm:px-6 text-center pt-24 pb-12">
        <div className="inline-flex items-center gap-2 sm:gap-3 border border-blue-500/30 bg-blue-900/20 backdrop-blur-md px-3 py-1 sm:px-6 sm:py-2 rounded-full mb-6 sm:mb-8 animate-in fade-in slide-in-from-top-4 duration-700">
           ...
           <span className="text-blue-200 text-[10px] sm:text-xs font-bold tracking-[0.2em] uppercase">Kocaeli & İst. Anadolu Yakası</span>
        </div>

        {/* FONT BOYUTLARI DÜZENLENDİ: text-4xl -> text-5xl */}
        <h1 className="text-5xl sm:text-6xl md:text-7xl lg:text-8xl font-black text-white mb-6 sm:mb-8 leading-[0.9] sm:leading-[0.95] tracking-tight drop-shadow-2xl animate-in fade-in zoom-in duration-700">
          GÜVENLİ TAŞIMA <br/>
        ...
        {/* Butonlar mobilde w-full yapıldı */}
        ...
        <div className="mt-10 sm:mt-20 flex flex-wrap justify-center gap-4 sm:gap-16 border-t border-white/10 pt-6 sm:pt-8 animate-in fade-in duration-1000 delay-500">
            {[
                { i: Clock, t: "15 Dk Varış" },
                { i: ShieldCheck, t: "Kaskolu Taşıma" },
                { i: MapPin, t: "7/24 Takip" }
            ].map((item, idx) => (
                <div key={idx} className="flex items-center gap-2 text-slate-400 bg-white/5 px-3 py-1 rounded-lg sm:bg-transparent">
                    <item.i className="text-blue-500" size={16} />
                    <span className="text-[10px] sm:text-sm font-bold uppercase tracking-wider">{item.t}</span>
                </div>
            ))}
        </div>
      </div>
